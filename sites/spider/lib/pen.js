...
VerletJS.prototype.move=function(){
	...
	if(web.isEnd()) {
		//重新产生路径，清除旧路径
		web.paths=genDaoPaths(web);
		web.curDest=web.paths[0];
		web.curIndex=0;
		/*pen.constraints.splice(1,pen.constraints.length-1);*/
		web.particles.forEach(function(particle){
			particle.arrive=false;
		})
		return;
	}
...
//从0开始
function inRect(x,y,width,height){
	if(!x||x<0||x>=width) return false;
	if(!y||y<0||y>=height) return false;
	return true;
}
//TODO 路径数组
function genDaoPaths(web){
	...
	//起点为中心点，然后随机走，100步
	var paths=new Array();
	var cur=new Vec2(parseInt(w1Num/2),parseInt(h1Num/2));
	var i=1000;
	while(i-->0){
		var x,y; 
		...
		do{
			var po=posbility[parseInt(Math.random()*posbility.length)];
			log("po",po)
			x=curX+po[0];
			y=curY+po[1];
		}while(!inRect(x,y,w1Num,h1Num))
		cur=new Vec2(x,y);
		paths.push(cur);
	}
	var paths=paths.map(function(vec2){
